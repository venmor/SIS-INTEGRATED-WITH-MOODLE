...
describe('policy (e2e)', () => {
  ...
  afterAll(async () => {
    ...
    await prisma.person.deleteMany({
      where: {
        displayName: { startsWith: 'E2E pol ' },
        accounts: { none: {} },
      },
    });
  ...
  it('requires a real approver who is never the target', async () => {
    ...
    const missing = await admin
      ...
      .set(CSRF)
      .send({
        ...grantBody(target.username, mweene.id, `E2E pol grant ${stamp}`),
  ...
  it('replays identical grants from one idempotency key without duplication', async () => {
    ...
    expect(replay.newState).toMatchObject({
      assignmentId: first.body.assignmentId,
    });
  ...
  it('resolves grant targets for grantors, empty for unknown, uniform for others', async () => {
    ...
    const found = await admin
      .post('/auth/grants/resolve')
      .set(CSRF)
      .send({ username: 'chanda.k' });
    ...
    const denied = await user
      .post('/auth/grants/resolve')
      .set(CSRF)
      .send({ username: 'mutinta.l' });
    expect(denied.status).toBe(403);
    expect(denied.body.message).toBe(GRANT_DENIED);
    const deniedGhost = await user
      .post('/auth/grants/resolve')
      .set(CSRF)
      .send({ username: `ghost.${stamp}` });
  ...
  it('audits guard rejections and records purpose plus prior/new refs on grants', async () => {
    ...
    const created = await admin
      .post('/auth/grants')
      .set(CSRF)
      .send(grantBody(target.username, mweene.id, reason));
    expect(created.status).toBe(201);
    const row = await prisma.auditEvent.findFirstOrThrow({
      where: {
        action: 'CMD-IAM-GrantRole',
        outcome: 'ALLOW',
        targetRef: created.body.assignmentId,
      },
    });
    expect(row.purpose).toBe(reason);
    expect(row.activeRole).toBe('SYSADMIN');
    expect(row.priorState).toBeNull();
    expect(row.newState).toMatchObject({
      assignmentId: created.body.assignmentId,
      role: 'TUT',
    });
  });
  ...
  it('emits the full §12.9 event chain and records the idempotency key', async () => {
    ...
    const created = await admin
      ...
      .set(CSRF)
      .send({
        ...grantBody(target.username, mweene.id, reason, {
          idempotencyKey: key,
        }),
        endsAt: '2026-12-31',
      });
