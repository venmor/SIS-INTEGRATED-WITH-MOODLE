...
<!-- Message ID: cd7a1691-b57f-5c01-8736-d897336fc1b7; chronological message: 97. -->

The dean’s experience must be unique. Creating a generic “staff user” and merely granting extra permissions would recreate the same disorganized experience we are trying to replace.

The correct model is:

> **One person identity + one or more role assignments + an organizational scope + a role-specific workspace.**

If a dean is also a lecturer or supervisor, they use the same account but explicitly switch workspaces. Every action is evaluated against the active role and scope.
...
so the way we have describe the deans experience thouroughly is how we were suppose to describe all the users and when performing each action available to them,not vague actor to action description,the ai agents needs to know what they developing on architecture level and design level and ux ui level,there patterns,and one action on a an archeture level may look simple but on a experience design level it needs more clarification
